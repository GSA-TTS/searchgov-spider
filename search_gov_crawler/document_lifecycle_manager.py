...
def run_stale_document_deletion(searchgov_settings: SearchgovSettings):
    ...
    for idx, docs in enumerate(batched(matching_docs, opensearch.batch_size), start=1):
        ...
        search_docs_not_found = [
            search_error for search_error in search_errors if search_error["delete"].get("result") == "not_found"
        ]
        ...
        total_errors += len(search_errors) + len(freshness_errors) - len(search_docs_not_found)
