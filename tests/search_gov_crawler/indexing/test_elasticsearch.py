...
def fixture_mock_convert_html(mocker):
    return mocker.patch("search_gov_crawler.indexing.transform.convert_html")


def test_add_to_batch(mocker, mock_convert_html, sample_spider):
    test_document = {"_id": "1", "title": "Test Document"}
    mock_update_dap_visits = mocker.patch("search_gov_crawler.indexing.es.update_dap_visits_to_document")
    ...
    mock_batch_upload = mocker.patch.object(es_uploader, "batch_upload")
    ...
    mock_batch_upload.assert_called_once_with(sample_spider)
...
def test_client_property(mocker, search_gov_es, mock_es_client):
    mock_es = mocker.patch("search_gov_crawler.indexing.es.Elasticsearch")
...
def test_batch_upload_with_errors(mocker, search_gov_es, sample_spider):
    mock_bulk = mocker.patch("search_gov_crawler.indexing.es.helpers.parallel_bulk")
...
def test_batch_upload_exception(mocker, search_gov_es, sample_spider) -> None:
    mock_bulk = mocker.patch("search_gov_crawler.indexing.es.helpers.parallel_bulk")
